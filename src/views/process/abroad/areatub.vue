<template>
  <el-card>
    ...
    <el-form ref="forms" :model="form" :inline="true">
      ...
      <el-form-item label="工作单位" v-if="checkRole(['admin'])">
        <el-select
          v-model="form.deptId"
          placeholder="工作单位"
          size="small"
      ...
      >
        <template #default="scope">
          ...
          <el-button
            ...
            type="text"
            v-if="
              (scope.row.applyStauts === 1 ||
                scope.row.applyStauts === 3 ||
                scope.row.applyStauts === 5 ||
                scope.row.applyStauts === 4) &&
              activeName === '0' &&
    ...
    >
      ...
      <el-row class="mt-20" :gutter="20">
        <el-col :span="8">申请编号: {{ abroadDetail.applyNo }}</el-col>
        <el-col :span="8">姓名: {{ abroadDetail.name }}</el-col>
        <el-col :span="8">身份证号: {{ abroadDetail.sfzh }}</el-col>
      </el-row>
      <el-row class="mt-20" :gutter="20">
        <el-col :span="8">工作单位: {{ abroadDetail.zgdw }}</el-col>
        <el-col :span="8">所在科室: {{ abroadDetail.ks }}</el-col>
        <el-col :span="8">职务: {{ abroadDetail.zw }}</el-col>
      </el-row>
      <el-row class="mt-20" :gutter="20">
        <el-col :span="8">性别: {{ genderEnum[abroadDetail.gender] }}</el-col>
        <el-col :span="8"
          >民族:{{
            filterType(abroadDetail.nation, lajw_ft_nation, "label", "value")
          }}
        </el-col>
        <el-col :span="8">籍贯: {{ abroadDetail.nativePlace }}</el-col>
      </el-row>
      <el-row class="mt-20" :gutter="20">
        <el-col :span="8">出生年月: {{ abroadDetail.csrq }}</el-col>
        <el-col :span="8"
          >参加工作时间: {{ formDate(abroadDetail.cjgzsj) }}</el-col
        >
        <el-col :span="8">学历: {{ abroadDetail.xl }}</el-col>
      </el-row>
      <el-row class="mt-20" :gutter="20">
        <el-col :span="8">党派: {{ abroadDetail.dp }}</el-col>
        <el-col :span="8">同行人员 : {{ abroadDetail.txry }}</el-col>
        <el-col :span="8"
          >起止时间:
      ...
      <el-row class="mt-20" :gutter="20">
        <el-col :span="8">出国(境)事由 : {{ abroadDetail.cgsy }}</el-col>
        <el-col :span="8">出国(境)目的地: {{ abroadDetail.spmdd }}</el-col>
        <el-col :span="8"
          >使用护照:
          ...
          <el-col :span="20">
            <el-table :data="changeTravels" style="width: 100%">
              ...
              >
                <template #default="scope">
                  <span
                    :style="{ color: scope.row.bgStatus == 5 ? '' : 'red' }"
                    >{{ bgStatus[scope.row.bgStatus] }}</span
                  >
                </template>
              </el-table-column>
            </el-table>
          ...
          <div>
            ...
            >
              <el-checkbox-group v-model="reviewForm.bdcjnrs">
                <div v-for="(item, key) in cantReview" :key="key">
                  <el-checkbox
            ...
            >
              <el-checkbox-group v-model="reviewForm.bycjnrs">
                <div v-for="(item, key) in uncantReview" :key="key">
                  <el-checkbox
                    class="checkbox-inner"
                    :label="key"
      ...
      <el-form ref="tripFormref" :model="tripForm">
        <el-form-item>
          <div class="flex-column-start">
            <el-checkbox v-model="tripForm.isTime" label="1"
              >起止时间</el-checkbox
            >
            ...
            <el-form-item v-if="tripForm.isMdd">
              <div class="flex-column-start ml-20">
                ...
                <span
                  ...
                  >暂无可更换证照</span
                >
                <template v-else-if="tripForm.isZz">
                  <el-checkbox-group
...
export default defineComponent({
  ...
  setup() {
    ...
    function tripSubmit() {
      if (!tripForm.isTime && !tripForm.isMdd) {
        return proxy.$modal.msgWarning("请至少选择一项变更");
      } else if (tripForm.isTime && !tripForm.startEndTime) {
        return proxy.$modal.msgWarning("请选择起止时间");
      } else if (tripForm.isMdd && !tripForm.bghmdd.join(",")) {
        return proxy.$modal.msgWarning("请选择目的地");
      } else if (tripForm.isZz && !tripForm.list?.length) {
        return proxy.$modal.msgWarning("请选择证照");
      }
    ...
    function reportedAction({ instanceId, applyNo, name }) {
      ...
      reportedForm.isQggb = 2; // qg:1 fgq:2
    ...
    function receiveReports() {
      proxy.$refs["approveFormRef"].validate(valid => {
        if (valid) {
          ...
          notReviewAbroad(params).then(res => {
            receiveDialog.value = !receiveDialog.value;
    ...
    function changeIsMdd(value) {
      if (value !== 1) {
        tripForm.isZz = undefined;
        tripForm.list = [];
      }
    }
    ...
    function handleAbroad(val, id, tId) {
      ...
      if (isAdd.value && isAdd.value == 1) {
        ...
        getAbroadDetail(id).then(res => {
          ...
          abroadForm.leaveCountry.gender = res.data.info.gender + "";
          if (res.data.info.nation) {
          ...
          useLicense(prams).then(val => {
            licenseList.value = val.data;
            abroadForm.useCertificates = res.data.useCertificates.map(item => {
              return item.zzId;
            });
          });
    ...
    function handleChangeName(stringName) {
      ...
      abroadForm.leaveCountry.csrq = proxy
        .$dayjs(selectInfo.csrq)
        .format("YYYY-MM-DD");
          ...
          if (isAdd.value && isAdd.value === 1) {
            ...
            editInfo(querParms.info).then(() => {
              ...
              const spPrams = {
                taskId: taskIds.value,
                result: 0,
                userName: abroadForm.fgld
              };
              notReviewAbroad(spPrams)
                .then(res => {
                  dialogVisible.value = !dialogVisible.value;
                  proxy.$tab.refreshPage();
    ...
    function getList() {
      ...
      if (activeName.value === "0") {
        getLists = getAbroadWaitList(params);
      } else if (activeName.value === "1") {
        getLists = getCreateByme(params);
      } else if (activeName.value === "2") {
        getLists = getHasCreate(params);
      } else {
        getLists = getAllListByFQg(params);
      }
      getLists.then(res => {
        loading.value = false;
    ...
    const handleDownloadTemplate = () => {
      downloadFile(downloadFqgTemplate, "国家工作人员因私出国（境）联审审批表");
    };
