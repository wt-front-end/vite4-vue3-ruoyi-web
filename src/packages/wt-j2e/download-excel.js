/*
 * @Author: xkloveme
 * @Date: 2022-04-22 15:38:46
 * @LastEditTime: 2022-05-11 15:54:38
 * @LastEditors: xkloveme
 * @Description: 抽离方法,当然你可以在文件中引入,并且使用
 * @FilePath: /la-ft-web/src/packages/wt-j2e/download-excel.js
 * @Copyright © xkloveme
 */
/**
 *
 ...
 */
export default function EXDownloadManager(
  sheets,
  beforeChange,
  fileName,
  fileSuffix
  ...
  sheets.forEach((item, sheetIndex) => {
    ...
    columns.forEach((column, columnIndex) => {
      ...
      if (Object.keys(column || {}).includes("isHead")) {
        isHead = column.isHead;
      }
      // 默认样式
      var defaultStyle = {};
      if (keys.includes("colWidth")) {
        defaultStyle = { colWidth: column.style.colWidth };
      }
      // 单元格数据
      var itemData = {
        ...column,
    ...
    dataSource.forEach(item => {
      ...
      columns.forEach((column, columnIndex) => {
        // 获取列数据
        var columnData = GetColumnData(item, column.field || column.dataIndex);
        // 单元格数据
        var itemData = {
          ...column,
          data: columnData,
...
function EXDownload(sheets, fileName, fileSuffix) {
  ...
  // 检查是否有数据
  if (!EXSheets || !EXSheets.length) {
    return;
  }

  // 文件名
  var EXFileName = fileName || "模版";
  ...
  var EXSheetStrings = [];
  // 下拉框 数组
  var EXSheetRangeList = [];
  // 便利 Worksheet
  EXSheets.forEach((sheet, sheetIndex) => {
    // 没有指定文件名称，则组装一个文件名称
    if (!fileName) {
      EXFileName += `${sheet.name}${sheetIndex ? "-" : ""}`;
    }
      ...
      row.forEach((cell, cellIndex) => {
        ...
        if (rowIndex === 0) {
          ...
          if (cell.style.colWidth === 0 || !!cell.style.colWidth) {
            // 更换 Row 头部
            EXSheetColumnString += `<Column ss:Index="${
              cellIndex + 1
            }" ss:AutoFitWidth="0" ss:Width="${cell.style.colWidth}"/>`;
          }
        }

        // 组合 StyleID
        var styleID = `s${sheetIndex}-${rowIndex}-${cellIndex}`;
        console.log(styleID, row, cell, 222);
        // 如果存在头部,就替换EXSheetRowHeadString,生成一个新头
        if (cell.isHead && rowIndex === 0) {
          // 获取单元格 Style 样式
          EXSheetRowHeadString = `<Row><Cell ss:StyleID="${styleID}" ss:MergeAcross="${
            row.length - 2
          }"><Data ss:Type="String">${cell.title}</Data></Cell></Row><Row>`;
        }
        // 获取数据类型
        var dataType = cell.dataType || typeof cell.title;
        ...
        // 超过 11 位的数字需要转成字符串
        if (dataType === "Number" && cell.title > 10000000000) {
          dataType = "String";
        }
        ...
        // 是否存在有下拉选项
        if (cell.rangeList) {
          EXSheetRangeList += sheet.isHead
            ? `<DataValidation xmlns="urn:schemas-microsoft-com:office:excel">
          <Range>C${cellIndex}</Range>
          <Type>List</Type>
          <CellRangeList/>
          <Value>${
            Array.isArray(cell.rangeList)
              ? "&quot;" + cell.rangeList.join(",") + "&quot;"
              : cell.rangeList
          }</Value>
         </DataValidation><DataValidation xmlns="urn:schemas-microsoft-com:office:excel"><Range>R1,R2</Range><ErrorStyle>Stop</ErrorStyle></DataValidation>`
            : `<DataValidation xmlns="urn:schemas-microsoft-com:office:excel"><Range>R1C${
                cellIndex + 1
              }</Range></DataValidation><DataValidation xmlns="urn:schemas-microsoft-com:office:excel">
          <Range>C${cellIndex + 1}</Range>
          <Type>List</Type>
          <CellRangeList/>
          <Value>${
            Array.isArray(cell.rangeList)
    ...
    // 添加下拉
    EXSheetString += EXSheetRangeList;
    // Worksheet 尾部
...
function EXStyle(cell, styleID) {
  ...
  // Date 类型样式处理
  if (cell.dataType === "Date" && cell.title) {
    // 有样式
    isStyle = true;
  ...
  if (styleKeys.length) {
    ...
    if (isColor || isFontSize || isFontName || isFontBold) {
      ...
      if (isFontName) {
        EXStyleSubString += ` ss:FontName="${style.fontName}"`;
      }
      EXStyleSubString += ' x:CharSet="134"';
      if (isFontSize) {
        EXStyleSubString += ` ss:Size="${style.fontSize}"`;
      }
      if (isColor) {
        EXStyleSubString += ` ss:Color="${style.color}"`;
      }
      if (isFontBold) {
        EXStyleSubString += ` ss:Bold="${style.fontBold}"`;
      }
      EXStyleSubString += "/>";
      // 添加 Font 样式
    ...
    if (isAlignmentHor || isAlignmentVer) {
      ...
      isStyle = true;
      // Alignment 样式内容
      EXStyleSubString = "<Alignment";
      if (isAlignmentHor) {
        EXStyleSubString += ` ss:Horizontal="${style.alignmentHor}"`;
      }
      if (isAlignmentVer) {
        EXStyleSubString += ` ss:Vertical="${style.alignmentVer}"`;
      }
      EXStyleSubString += "/>";
      // 添加 Font 样式
      EXStyleString += EXStyleSubString;
    ...
    if (isBorderColor || isBorderWidth || isBorderStyle) {
      ...
      var borderWidth = `${
        style.borderWidth === 0 ? 0 : style.borderWidth || 1
      }`;
      ...
      if (borderWidths.length === 1) {
        var temp1 = borderWidths[0];
        borderWidths = [temp1, temp1, temp1, temp1];
      }
      if (borderStyles.length === 1) {
        var temp2 = borderStyles[0];
        borderStyles = [temp2, temp2, temp2, temp2];
      }
      // 取最小值
      var count = Math.max(
        Math.min(
...
function EXCell(cell, dataType, styleID, rowIndex) {
  ...
  if (isMerges) {
    ...
    var merge =
      merges.find(
        merge =>
          (!merge.row && Number(merge.row) !== 0) ||
          Number(merge.row) === rowIndex
      ) || {};
    // 有合并样式
    if (merge) {
      // 样式 Keys
      ...
      // 横向合并单元格
      if (isMergeHor) {
        EXCellString += ` ss:MergeAcross="${merge.hor}"`;
      }
      // 竖向合并单元格
      if (isMergeVer) {
        EXCellString += ` ss:MergeDown="${merge.ver}"`;
      }
    }
  }
  // Cell 头部结束符
